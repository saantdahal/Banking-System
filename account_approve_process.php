...
if (isset($_POST['approve_cust'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $duplicate_check_sql = "SELECT * FROM pending_accounts WHERE Mobile_no = '$mob_no' AND Application_no != '$application_no'";
        $duplicate_check_result = $conn->query($duplicate_check_sql);
        if ($duplicate_check_result->num_rows > 0) {
            echo '<script>alert("Duplicate record found with the same Mobile number. Account cannot be created.")</script>';
        } else {
            // Check for duplicate PAN number
            $duplicate_check_sql = "SELECT * FROM pending_accounts WHERE PAN = '$pan' AND Application_no != '$application_no'";
            $duplicate_check_result = $conn->query($duplicate_check_sql);
            if ($duplicate_check_result->num_rows > 0) {
                echo '<script>alert("Duplicate record found with the same PAN number. Account cannot be created.")</script>';
            } else {
                // Check for duplicate Citizenship number
                $duplicate_check_sql = "SELECT * FROM pending_accounts WHERE CITIZENSHIP = '$citizenship' AND Application_no != '$application_no'";
                $duplicate_check_result = $conn->query($duplicate_check_sql);
                if ($duplicate_check_result->num_rows > 0) {
                    echo '<script>alert("Duplicate record found with the same Citizenship number. Account cannot be created.")</script>';
                } else {
